...
- 2026-09-16: agy REVIEW-03 fix (SP-1, SP-2, SP-D1, ST-D1) tamamlandı (TDD). `elif yol.exists(): pass` kaldırıldı; boş/whitespace çıktının eski dolu dosyayla başarıya çevrilmesi engellendi (SP-1). Tüm dosya erişim/varlık/okuma/yazma kontrolleri `try...except (OSError, UnicodeError)` bloğuna alındı; `chmod(0)` iş dizininde PermissionError kaçışı önlendi (SP-2). UTF-8 surrogate kodlama hatası (`UnicodeEncodeError`) tanımlı redde dönüştürüldü (SP-D1). str ve bytes dalları tek I/O hata sınırında birleştirildi (ST-D1). Birincil seam (`isi_ilerlet`) ve `SurucuAdim` üzerinden regresyon testleri eklendi.

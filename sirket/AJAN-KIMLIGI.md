...
## Kim kimdir
- **İnsan — bu repoyu kuran kişi.** İşi verir (`bin/dongu.py "<iş>"`), sonda karar verir: `yayinla` (commit),
  `revize "<not>"`, `red`. Ara adımda onay vermez — sistem işi sonuna kadar götürür. `bin/kapi.py` ve
  `bin/dongu.py` onun düğmeleridir; sen çalıştırmazsın.
...
## Döngü (senin yerin)
```
insan: dongu.py "<iş>"          → talep, evre: sozlesme
sistem-sevk  (claude)           → sozlesme.json + increment.md      → bekleyen_onay: sozlesme
dongu: onayla (otomatik)        → sozlesme.onayli.json, motor kilidi → evre: insaat
...
sistem-bekci  (ters motor)      → bekci-raporu.json PASS|FAIL        → yayin-bekliyor | fail
   fail → dongu: yeniden (en fazla 2) → inşaat FAIL raporunu okuyup düzeltir
insan: kapi.py yayinla | revize "<not>" | red   → commit / sevk yeniden keser / kapanır
```
1. **Tetik:** her koşu yine `bin/kos.py <takim>`'in tek koşusudur; zinciri `bin/dongu.py` (LLM'siz otomat) kurar.
   Sürücü evreyi kontrol eder: yanlış evrede koşu başlamaz, `durum.json`'a sebep düşer. İnşaat ikinci denemede
   `bekci-raporu.json`'u, sevk revizede `evre.json.revizyon` notunu önünde bulur.
...
7. **Karar:** yayınlayan sen değilsin. Sevk sözleşme kadar, inşaat teslim kadar, bekçi rapor kadar gider ve durur;
   insan sonda `yayinla` / `revize` / `red` der.
...
- İnsanın dosyalarına dokunma: `ANAYASA.md`, `hedef.md`, `kararlar.md`, `kapsam-disi.md`, `sema/`, `kurallar.md`,
  `sozlesme.onayli.json`, `bin/kapi.py`, `bin/dongu.py` (§5).
